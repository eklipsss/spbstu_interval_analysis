...
def draw_Tol(A, b, max_x, max_Tol, name: str):
    ...
    ax.plot_surface(x_1_, x_2_, np.array(list_tol), cmap='viridis',
                    edgecolor='green')
    ax.set_title(f"Tol {name}")
    ...
    match correction:
        case "Ab":
            ...
            for i in range(len(As)):
                ...
                ip.IntLinIncR2(A_, b_, consistency='tol', title=f"source/Corr/Ab-correction {i}", save=True,
                               size=(8, 8), show=False)
                # plt.scatter(1, 2)
                # plt.show()
                ...
                # draw_Tol(A_, b_, maxX, maxTol, name=f"source/Tol/Ab-correction {i}")
        case "A":
            ...
            for i in range(len(As)):
                ...
                ip.IntLinIncR2(A_, b_, consistency='tol', title=f"source/Corr/A-correction {i}", save=True,
                               size=(8, 8), show=False)
                # plt.scatter(1, 2)
                # plt.show()
                ...
                # draw_Tol(A_, b_, maxX, maxTol, name=f"source/Tol/A-correction {i}")
        case "b":
            ...
            for i in range(len(As)):
                ...
                print(b_)
                print(emptinessTol(A_, b_))
                ip.IntLinIncR2(A_, b_, consistency='tol', title=f"source/Corr/b-correction {i}", save=True,
                               size=(8, 8), show=False)
                # plt.scatter(1, 2)
                # plt.show()
                ...
                # draw_Tol(A_, b_, maxX, maxTol, name=f"source/Tol/b-correction {i}")
